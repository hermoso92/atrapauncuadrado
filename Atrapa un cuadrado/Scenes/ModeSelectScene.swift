...
final class ModeSelectScene: BaseScene {
    ...
    override init(sceneSize: CGSize, gameMode: GameMode? = nil) {
        super.init(sceneSize: sceneSize, gameMode: nil)
    }
    ...
    private func buildScene() {
        ...
        setupBackdrop(
            ...
            subtitle: "Tres rutas claras. Entra rapido, no te pelees con el menu."
        )
        ...
        strip.position = CGPoint(x: size.width / 2, y: size.height - 222)
        addChild(strip)
        ...
        let summary = makeLabel(
            text: "Ultima ruta: \(progress.lastSelectedMode.title)  •  Toca una tarjeta para entrar.",
        ...
        let cardSize = CGSize(width: size.width - 46, height: 126)
        let startY = size.height - 356
        let spacing: CGFloat = 144
        ...
        let footer = makeLabel(
            text: "Clasico para leer. Arsenal para montar build. Fantasma para sufrir con estilo.",
            fontNamed: GameConfig.coinFont,
            fontSize: 11,
    ...
    private func modeCard(for mode: GameMode, size: CGSize) -> SKShapeNode {
        ...
        let accentBar = SKShapeNode(rectOf: CGSize(width: 10, height: size.height - 26), cornerRadius: 5)
        ...
        let iconPanel = SKShapeNode(rectOf: CGSize(width: 74, height: 74), cornerRadius: 24)
        ...
        iconPanel.position = CGPoint(x: size.width / 2 - 60, y: 0)
        ...
        title.position = CGPoint(x: -size.width / 2 + 36, y: 34)
        card.addChild(title)

        let badge = SKShapeNode(rectOf: CGSize(width: 144, height: 26), cornerRadius: 13)
        ...
        badge.lineWidth = 1.2
        badge.position = CGPoint(x: -size.width / 2 + 106, y: 6)
        card.addChild(badge)
        ...
        hook.position = CGPoint(x: -size.width / 2 + 36, y: -24)
        card.addChild(hook)

        let actionPlate = SKShapeNode(rectOf: CGSize(width: 94, height: 24), cornerRadius: 12)
        ...
        actionPlate.position = CGPoint(x: size.width / 2 - 60, y: -42)
        ...
        action.position = CGPoint(x: 0, y: 7)
    ...
    private func modeGlyph(for mode: GameMode, unlocked: Bool) -> SKNode {
        ...
        case .ghost:
            for index in 0..<3 {
                let slash = SKShapeNode(rectOf: CGSize(width: 30 - CGFloat(index) * 5, height: 4), cornerRadius: 2)
                slash.fillColor = unlocked ? mode.summaryAccent : Palette.textSecondary
                slash.strokeColor = .clear
                slash.position = CGPoint(x: 0, y: 10 - CGFloat(index) * 10)
